<footer
  class="content-info footer bg-brandblue text-sm md:text-lg lg:text-base xl:text-lg text-justify pt-5 pb-9 md:pt-7 md:pb-14">
  ...
  <div class="container space-y-8 md:space-y-12 lg:space-y-0 lg:flex lg:space-x-20 xl:space-x-24">
    <div class="lg:max-w-[300px] xl:max-w-[420px] 2xl:max-w-[485px]">
      <a class="inline-block font-bold text-2xl md:text-4xl mb-3 md:mb-3.5" href="{{ home_url('/') }}">
        @if ($logo_alt)
          <img src="{{ $logo_alt['url'] }}" alt="Logo Alt Umrohpass" class="h-[45px] md:h-20">
        @else
          <span>{!! $siteName !!}</span>
        @endif
      </a>
    ...
    <div id="contact" class="lg:max-w-[320px] lg:pt-8">
      @if ($contact_us)
        ...
        <div class="flex items-center space-x-4 lg:block lg:space-x-0 lg:space-y-5">
          @if ($whatsapp)
            <a href="{!! $whatsapp['number'] ? "https://wa.me/{$whatsapp['number']}" : 'javascript:void(0)' !!}" target="{{ $whatsapp['number'] ? '_blank' : '' }}"
              rel="noopener noreferrer" class="button button--whatsapp space-x-2 lg:w-full">
              <svg class="w-[18px] h-[18px] md:h-[24px] md:w-[24px] transition-all" width="24" height="24"
          ...
          @if ($social_media_links)
            <div class="flex items-center space-x-3.5 s:space-x-4 lg:justify-between">
              @foreach ($social_media_links as $link)
                <a href="{{ $link['url'] }}" target="_blank" rel="noopener noreferrer" class="social-media-link">
                  @if ($link['icon_svg'])
                    {!! $link['icon_svg'] !!}
                  @elseif ($link['icon'])
