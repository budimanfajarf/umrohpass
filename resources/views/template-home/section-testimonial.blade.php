@if ($testimonial_section)
  ...
  <section id="testimonial" style="background-image: url('{{ $background_image_url }}')"
    ...
    <div
      class="relative container flex flex-col space-y-10 lg:flex-row lg:items-center lg:space-x-10 2xl:space-x-28 lg:justify-between">
      @if ($testimonial_section['title'])
        <h3
          class="text-brandblack text-2xl leading-normal md:text-3xlp lg:text-2xl 2xl:text-3xlp font-bold mb-8 2xl:max-w-[400px]">
          ...
          @foreach ($testimonial_section['items'] as $item)
            @if ($item && $item['name'] && $item['testimony'])
              <div class="px-2.5 w-full">
                <div
                  class="w-full p-3 pt-7 pb-5 md:p-6 md:pb-7 bg-white rounded-ten max-w-[180px] md:max-w-[235px] mx-auto h-[230px] md:h-[300px]">
                  @php($photo_url = $item['photo']['url'] ?? "https://ui-avatars.com/api/?name={$item['name']}&background=C8A251&color=fff")

                  <img class="mb-3 md:mb-8 w-[70px] md:w-[85px] h-[70px] md:h-[85px] rounded-full mx-auto"
                    src="{{ $photo_url }}" alt="{{ $item['name'] }}">
